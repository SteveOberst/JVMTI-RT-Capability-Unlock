...
#define BUILTIN_OFFSETOF_REQUIRES_CPP                                                              \
    (_MSC_VER < 9999 ? 1 : 0) // Update this number if/when MS ever fix this
#else
#define BUILTIN_OFFSETOF_REQUIRES_CPP 0
#endif

#if (                                                                                              \
    (!BUILTIN_OFFSETOF_REQUIRES_CPP || defined(__cplusplus))                                       \
...
#define CONTAINING_RECORD(address, type, field)                                                    \
    ((type*)((PCHAR)(address) - (ULONG_PTR)(__builtin_offsetof(type, field))))
#endif
...
#define RTL_CONSTANT_STRING(s)                                                                     \
    { sizeof(s) - sizeof(*(s)), sizeof(s), (remove_constref<decltype(*(s))>::type*)(s) }
#define RTL_CONSTANT_ANSI_STRING(s) RTL_CONSTANT_STRING(s)
    ...
    typedef struct _SYSTEM_PROCESS_INFORMATION
    {
        ...
        // SYSTEM_EXTENDED_THREAD_INFORMATION + SYSTEM_PROCESS_INFORMATION_EXTENSION //
...
#define CODEINTEGRITYPOLICY_OPTION_RELOAD_POLICY_NO_REBOOT                                         \
    0x10000000 // NtSetSystemInformation reloads SiPolicy.p7b
...
typedef UCHAR PRIORITY_CLASS;
    ...
    typedef struct _PS_ATTRIBUTE
    {
        ...
        PSIZE_T
            ReturnLength; // Either 0 or specifies size of data returned to caller via "ValuePtr"
    } PS_ATTRIBUTE, *PPS_ATTRIBUTE;

    typedef struct _PS_ATTRIBUTE_LIST
    {
        SIZE_T TotalLength; // sizeof(PS_ATTRIBUTE_LIST) + <attribute count> * sizeof(PS_ATTRIBUTE)
        PS_ATTRIBUTE Attributes[1]; // Depends on how many attribute entries should be supplied to
                                    // NtCreateUserProcess
...
#define PROCESS_HANDLE_TRACING_MAX_SLOTS 0x20000
    ...
    typedef enum _PROCESSINFOCLASS
    {
        ...
        ProcessHandleTracing,    // q: PROCESS_HANDLE_TRACING_QUERY; s: size 0 disables, otherwise
                                 // enables
        ...
        ProcessManageWritesToExecutableMemory, // MANAGE_WRITES_TO_EXECUTABLE_MEMORY // since
                                               // REDSTONE3
        ...
        ProcessAltSystemCallInformation, // qs: BOOLEAN (kernel-mode only) // INT2E // since 20H1 //
                                         // 100
    ...
    typedef enum _SYSTEM_INFORMATION_CLASS
    {
        ...
        SystemProcessorPerformanceInformation, // q: SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION (EX
        ...
        SystemExtendServiceTableInformation,   // s (requires SeLoadDriverPrivilege) // loads win32k
                                               // only
        SystemPrioritySeperation,              // s (requires SeTcbPrivilege)
        SystemVerifierAddDriverInformation,    // s (requires SeDebugPrivilege) // 40
        SystemVerifierRemoveDriverInformation, // s (requires SeDebugPrivilege)
        SystemProcessorIdleInformation,   // q: SYSTEM_PROCESSOR_IDLE_INFORMATION (EX in: USHORT
                                          // ProcessorGroup)
        ...
        SystemLoadGdiDriverInSystemSpace,     // s: SYSTEM_GDI_DRIVER_INFORMATION (kernel-mode only)
                                              // (same as SystemLoadGdiDriverInformation)
        ...
        SystemProcessorPowerInformation,     // q: SYSTEM_PROCESSOR_POWER_INFORMATION (EX in: USHORT
                                             // ProcessorGroup)
        ...
        SystemWatchdogTimerInformation,    // q: SYSTEM_WATCHDOG_TIMER_INFORMATION // (kernel-mode
                                           // only)
        SystemLogicalProcessorInformation, // q: SYSTEM_LOGICAL_PROCESSOR_INFORMATION (EX in: USHORT
                                           // ProcessorGroup)
        ...
        SystemCoverageInformation,        // q: COVERAGE_MODULES s: COVERAGE_MODULE_REQUEST //
                                          // ExpCovQueryInformation (requires SeDebugPrivilege)
        SystemPrefetchPatchInformation,   // SYSTEM_PREFETCH_PATCH_INFORMATION
        SystemVerifierFaultsInformation,  // s: SYSTEM_VERIFIER_FAULTS_INFORMATION (requires
                                          // SeDebugPrivilege)
        SystemSystemPartitionInformation, // q: SYSTEM_SYSTEM_PARTITION_INFORMATION
        SystemSystemDiskInformation,      // q: SYSTEM_SYSTEM_DISK_INFORMATION
        SystemProcessorPerformanceDistribution, // q: SYSTEM_PROCESSOR_PERFORMANCE_DISTRIBUTION (EX
                                                // in: USHORT ProcessorGroup) // 100
        SystemNumaProximityNodeInformation,     // q; s: SYSTEM_NUMA_PROXIMITY_MAP
        SystemDynamicTimeZoneInformation,       // q; s: RTL_DYNAMIC_TIME_ZONE_INFORMATION (requires
                                                // SeTimeZonePrivilege)
        ...
        SystemLogicalProcessorAndGroupInformation, // q: SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX (EX
                                                   // in: LOGICAL_PROCESSOR_RELATIONSHIP
                                                   // RelationshipType) // since WIN7 //
                                                   // KeQueryLogicalProcessorRelationship
        SystemProcessorCycleTimeInformation, // q: SYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION[] (EX in:
                                             // USHORT ProcessorGroup)
        SystemStoreInformation,              // q; s: SYSTEM_STORE_INFORMATION (requires
                                // SeProfileSingleProcessPrivilege) // SmQueryStoreInformation
        ...
        SystemAcpiAuditInformation,        // q: SYSTEM_ACPI_AUDIT_INFORMATION //
                                    // HaliQuerySystemInformation -> HalpAuditQueryResults, info
                                    // class 26
        ...
        SystemPlatformBinaryInformation,         // q: SYSTEM_PLATFORM_BINARY_INFORMATION (requires
                                                 // SeTcbPrivilege)
        ...
        SystemProcessorPerformanceInformationEx,   // q: SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION_EX
                                                   // // (EX in: USHORT ProcessorGroup) // since
                                                   // WINBLUE
        ...
        SystemProcessorCycleStatsInformation, // q: SYSTEM_PROCESSOR_CYCLE_STATS_INFORMATION (EX in:
                                              // USHORT ProcessorGroup) // 160
        ...
        SystemHypervisorSharedPageInformation, // SYSTEM_HYPERVISOR_SHARED_PAGE_INFORMATION // since
                                               // REDSTONE4
        ...
        SystemFeatureConfigurationInformation, // SYSTEM_FEATURE_CONFIGURATION_INFORMATION // since
                                               // 20H1 // 210
        ...
        SystemSingleProcessorRelationshipInformation, // q: SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX
    ...
    typedef enum _THREADINFOCLASS
    {
        ...
        ThreadTebInformation,     // q: THREAD_TEB_INFORMATION (requires THREAD_GET_CONTEXT +
                                  // THREAD_SET_CONTEXT)
        ThreadCSwitchMon,
        ThreadCSwitchPmu,
        ...
        ThreadCpuAccountingInformation, // q: BOOLEAN; s: HANDLE (NtOpenSession) // NtCurrentThread
                                        // // since WIN8
        ...
        ThreadManageWritesToExecutableMemory, // MANAGE_WRITES_TO_EXECUTABLE_MEMORY // since
                                              // REDSTONE3
...
#define JobObjectBasicAccountingInformation                                                        \
    ((JOBOBJECTINFOCLASS)1) // JOBOBJECT_BASIC_ACCOUNTING_INFORMATION
...
#define JobObjectEndOfJobTimeInformation                                                           \
    ((JOBOBJECTINFOCLASS)6) // JOBOBJECT_END_OF_JOB_TIME_INFORMATION
#define JobObjectAssociateCompletionPortInformation                                                \
    ((JOBOBJECTINFOCLASS)7) // JOBOBJECT_ASSOCIATE_COMPLETION_PORT
#define JobObjectBasicAndIoAccountingInformation                                                   \
    ((JOBOBJECTINFOCLASS)8) // JOBOBJECT_BASIC_AND_IO_ACCOUNTING_INFORMATION
#define JobObjectExtendedLimitInformation                                                          \
    ((JOBOBJECTINFOCLASS)9)                                 // JOBOBJECT_EXTENDED_LIMIT_INFORMATION
...
#define JobObjectNotificationLimitInformation                                                      \
    ((JOBOBJECTINFOCLASS)12) // JOBOBJECT_NOTIFICATION_LIMIT_INFORMATION
#define JobObjectLimitViolationInformation                                                         \
    ((JOBOBJECTINFOCLASS)13) // JOBOBJECT_LIMIT_VIOLATION_INFORMATION
#define JobObjectGroupInformationEx ((JOBOBJECTINFOCLASS)14) // GROUP_AFFINITY (ARRAY)
#define JobObjectCpuRateControlInformation                                                         \
    ((JOBOBJECTINFOCLASS)15) // JOBOBJECT_CPU_RATE_CONTROL_INFORMATION
...
#define JobObjectInterferenceInformation                                                           \
    ((JOBOBJECTINFOCLASS)26) // JOBOBJECT_INTERFERENCE_INFORMATION
#define JobObjectClearPeakJobMemoryUsed ((JOBOBJECTINFOCLASS)27)
#define JobObjectMemoryUsageInformation                                                            \
    ((JOBOBJECTINFOCLASS)28) // JOBOBJECT_MEMORY_USAGE_INFORMATION //
...
#define JobObjectNetRateControlInformation                                                         \
    ((JOBOBJECTINFOCLASS)32) // JOBOBJECT_NET_RATE_CONTROL_INFORMATION
#define JobObjectNotificationLimitInformation2                                                     \
    ((JOBOBJECTINFOCLASS)33) // JOBOBJECT_NOTIFICATION_LIMIT_INFORMATION_2
#define JobObjectLimitViolationInformation2                                                        \
    ((JOBOBJECTINFOCLASS)34) // JOBOBJECT_LIMIT_VIOLATION_INFORMATION_2
...
#define REG_MAX_KEY_NAME_LENGTH 512
    ...
    {
        ULONG VirtualizationCandidate : 1; // Tells whether the key is part of the virtualization
                                           // namespace scope (only HKLM\Software for now).
        ULONG VirtualizationEnabled : 1; // Tells whether virtualization is enabled on this key. Can
                                         // be 1 only if above flag is 1.
        ULONG VirtualTarget : 1; // Tells if the key is a virtual key. Can be 1 only if above 2 are
                                 // 0. Valid only on the virtual store key handles.
        ULONG VirtualStore : 1;  // Tells if the key is a part of the virtual store path. Valid only
                                 // on the virtual store key handles.
        ULONG VirtualSource : 1; // Tells if the key has ever been virtualized, can be 1 only if
    ...
    typedef enum _FILE_INFORMATION_CLASS
    {
        ...
        FileRenameInformationBypassAccessCheck, // (kernel-mode only); FILE_RENAME_INFORMATION //
                                                // since WIN8
        ...
        FileDesiredStorageClassInformation,       // FILE_DESIRED_STORAGE_CLASS_INFORMATION // since
                                                  // REDSTONE2
...
#pragma pack(pop)
    ...
    static_assert(
        FIELD_OFFSET(KUSER_SHARED_DATA, AlternativeArchitecture) == 0x2c0,
        "Offset check");
    ...
    static_assert(
        FIELD_OFFSET(KUSER_SHARED_DATA, LastSystemRITEventTickCount) == 0x2e4,
        "Offset check");
...
#define RTL_CONSTANT_OBJECT_ATTRIBUTES(n, a)                                                       \
    { sizeof(OBJECT_ATTRIBUTES),                                                                   \
      nullptr,                                                                                     \
      ((remove_constref<PUNICODE_STRING>::type)(n)),                                               \
      (a),                                                                                         \
      nullptr,                                                                                     \
      nullptr }
#endif
...
#define EVENT_ALL_ACCESS                                                                           \
    (STANDARD_RIGHTS_REQUIRED | SYNCHRONIZE | EVENT_QUERY_STATE | EVENT_MODIFY_STATE)
#endif
...
#define HeapOptimizeResources                                                                      \
    ((HEAP_INFORMATION_CLASS)0x3) // q; s: HEAP_OPTIMIZE_RESOURCES_INFORMATION
...
#define HeapSetDebuggingInformation                                                                \
    ((HEAP_INFORMATION_CLASS)0x80000002) // q; s: HEAP_DEBUGGING_INFORMATION
...
#define TOKEN_SECURITY_ATTRIBUTES_INFORMATION_VERSION                                              \
    ...
    NtCreateSectionEx(
        ...
        _In_opt_ HANDLE             FileHandle,
        _Inout_updates_opt_(
            ExtendedParameterCount) struct _MEM_EXTENDED_PARAMETER* ExtendedParameters,
...
#else
    // Allow Windows Vista/7 x86 to link (import '@8' decorated stdcall name from ntdll.lib instead
    // of '@20') If you're using a newer **x86 32 bit** version of Windows, use the previous
    // declaration (or make a typedef)
    NTSYSCALLAPI
    NTSTATUS
    NTAPI
        ...
        _Reserved_ PWORKER_FACTORY_DEFERRED_WORK DeferredWork)
    {
        ...
        return ZwWaitForWorkViaWorkerFactory(WorkerFactoryHandle, MiniPackets);
    }
#endif
#endif
...
BOOL NTAPI RtlQueryPerformanceCounter(_Out_ PLARGE_INTEGER PerformanceCount)
{
    ...
    NTSYSAPI
    VOID NTAPI
    RtlInitUnicodeString(_Out_ PUNICODE_STRING DestinationString, _In_opt_z_ PCWSTR SourceString);

    NTSYSAPI
    NTSTATUS
    NTAPI
    RtlInitUnicodeStringEx(_Out_ PUNICODE_STRING DestinationString, _In_opt_z_ PCWSTR SourceString);
    ...
    NTSYSAPI
    VOID WINAPI
    RtlRestoreContext(_In_ PCONTEXT ContextRecord, _In_opt_ PEXCEPTION_RECORD ExceptionRecord);
#endif
    ...
    NTSYSAPI
    VOID NTAPI
    PfxRemovePrefix(_In_ PPREFIX_TABLE PrefixTable, _In_ PPREFIX_TABLE_ENTRY PrefixTableEntry);
    ...
    NTSYSAPI
    _Success_(return != 0) USHORT NTAPI RtlCaptureStackBackTrace(
    ...
    NTSYSAPI
    VOID NTAPI
    RtlClearBits(_In_ PRTL_BITMAP BitMapHeader, _In_ ULONG StartingIndex, _In_ ULONG NumberToClear);
    ...
    NTSYSAPI
    LONG NTAPI
    RtlCompareAltitudes(_In_ PCUNICODE_STRING Altitude1, _In_ PCUNICODE_STRING Altitude2);
#endif
    ...
    NTSYSAPI
    VOID NTAPI
    RtlSetBits(_In_ PRTL_BITMAP BitMapHeader, _In_ ULONG StartingIndex, _In_ ULONG NumberToSet);
    ...
    NTSYSAPI
    VOID NTAPI
    TpCallbackSetEventOnCompletion(_Inout_ PTP_CALLBACK_INSTANCE Instance, _In_ HANDLE Event);
    ...
    NTSYSAPI
    VOID NTAPI
    TpCallbackReleaseMutexOnCompletion(_Inout_ PTP_CALLBACK_INSTANCE Instance, _In_ HANDLE Mutex);
    ...
    NTSYSAPI
    VOID NTAPI
    TpCallbackUnloadDllOnCompletion(_Inout_ PTP_CALLBACK_INSTANCE Instance, _In_ PVOID DllHandle);
    ...
    _Check_return_ NTSYSAPI NTSTATUS NTAPI TpSimpleTryPost(
    ...
    NTSYSAPI
    VOID NTAPI
    TpSetWait(_Inout_ PTP_WAIT Wait, _In_opt_ HANDLE Handle, _In_opt_ PLARGE_INTEGER Timeout);
